...
//
// 抓数据默认走 C 单标签软导航；也可选「新标签页抓取」：首页控制器保持不跳转，
//     商品子页在 document-start 监听页面自然发出的 get_rw，回传后确认关闭。
//     两种模式的导航故障都会熔断冷却，绝不高速重复领取任务。
//   风控签名是页面 JS 现场算的，伪造不了 —— 所以正解不是伪造上下文，
//   而是真的产生上下文（走 C）。这也正是「手动点商品正常」所证明的那条路。
//
// 时段配额：按「时段剩余时间 ÷ 时段剩余单数」动态算间隔，把单量均匀铺开。
//
// 通知：验证码发现时推一次；风控推两次；其它情况只推一次。
// ============================================================================
(function () {
  'use strict';
  ...
  const 更新安装地址 =
    'https://alist.allplat.top/sd/SCvxw21T';
  ...
  // error=266900002 + data:null：先等完整详情最多 10 秒；仍没有时，
  // 将这份原始响应上传给后台，由后台确认商品不存在，绝不能在客户端丢弃。
  const 商品不存在错误码 = 266900002;
  const 商品不存在确认等待毫秒 = 10 * 1000;
  // 人工验证码在任务过期后会放回/清理；为避免连续触发验证码，固定休眠两小时再领下一单。
  const 验证码超时休眠毫秒 = 2 * 60 * 60 * 1000;
  // 导航基础设施异常时的冷却阶梯。成功抓到有效 payload 才会清零。
  const 导航熔断冷却毫秒 = [10 * 60 * 1000, 30 * 60 * 1000, 2 * 60 * 60 * 1000];
  ...
  const 键 = {
    ...
    导航熔断: '__跑任务_导航熔断__', // { 级别, 截止, 原因 }，跨刷新也禁止高速重领
    验证码冷却: '__跑任务_验证码冷却__', // 人工验证超时后的固定两小时等待
    更新: '__跑任务_更新__',
  };
  ...
  function 取导航熔断() {
    const 原 = 读(键.导航熔断, null);
    if (!原 || typeof 原 !== 'object') return null;
    const 截止 = Number(原.截止);
    const 级别 = Math.min(导航熔断冷却毫秒.length,
      Math.max(1, Math.floor(Number(原.级别) || 1)));
    if (!Number.isFinite(截止) || 截止 <= 0) return null;
    return { 级别, 截止, 原因: String(原.原因 || '导航异常').slice(0, 240) };
  }

  function 触发导航熔断(原因) {
    const 旧 = 取导航熔断();
    // 冷却期过后允许一次恢复探测；若探测仍失败，级别继续升级。
    const 级别 = Math.min(导航熔断冷却毫秒.length, (旧 ? 旧.级别 : 0) + 1);
    const 截止 = 现在() + 导航熔断冷却毫秒[级别 - 1];
    const 熔断 = { 级别, 截止, 原因: String(原因 || '导航异常').slice(0, 240) };
    写(键.导航熔断, 熔断);
    记(`导航熔断 L${级别}：${熔断.原因}，冷却 ${导航熔断冷却毫秒[级别 - 1] / 60000} 分钟`, '错误');
    return 熔断;
  }
  ...
  function 清导航熔断(原因) {
    if (取导航熔断()) {
      删(键.导航熔断);
      记(原因 || '已成功抓到数据，导航熔断已清除');
    }
  }

  function 取验证码冷却() {
    const 原 = 读(键.验证码冷却, null);
    const 截止 = Number(原 && 原.截止);
    if (!Number.isFinite(截止) || 截止 <= 0) return null;
    if (截止 <= 现在()) {
      删(键.验证码冷却);
      return null;
    }
    return { 截止, 原因: String(原.原因 || '等待手动验证码超时').slice(0, 160) };
  }

  function 触发验证码冷却(原因) {
    const 冷却 = {
      截止: 现在() + 验证码超时休眠毫秒,
      原因: String(原因 || '等待手动验证码超时').slice(0, 160),
    };
    写(键.验证码冷却, 冷却);
    return 冷却;
  }
  ...
  // 手动点「启动任务」是明确的人工重试信号（iOS 无控制台时更是唯一入口）：
  // 一次性清掉所有会拦住领单的冷却/熔断，让本次点击真的能开工。
  // 单次点击不会造成高速重领——若这次仍撞验证码/导航失败，冷却会重新触发。
  // ⚠ 绝不能在「自动恢复」路径（整页刷新后由运行态拉起主循环）调用此函数，
  // 否则冷加载→自动恢复→立刻重领，熔断与验证码冷却都会形同虚设。
  function 解冻冷却(来源) {
    const 前缀 = 来源 ? 来源 + '，' : '';
    const 清了 = { 导航熔断: !!取导航熔断(), 验证码冷却: !!取验证码冷却() };
    清导航熔断(前缀 + '已清除导航熔断冷却');
    清验证码冷却(前缀 + '已清除验证码等待冷却');
    return 清了;
  }
  ...
  const 默认配置 = {
    ...
    // 服务端按账号统计的当天完成上限；每次 claim 前都会校验，防止多设备超额。
    每日完成上限: 400,
    抖动百分比: 65,      // 间隔上下浮动 ±65%，避免固定节奏
    // 每单从此范围随机抽一个「最短等待」，再与时段均摊节奏取较大值。
    间隔下限秒: 20,
    间隔上限秒: 48,
    抓取超时秒: 60,      // 等 get_rw 响应的上限

    // 默认以新标签页打开商品页抓取：首页控制器不跳转、抓完子页自关。
    // iOS/WKWebView 上 window.open 常不给句柄，已按「无句柄模式」处理。仍可切回单标签。
    抓取模式: '新标签页',
  ...
  function 取配置() {
    const 已存 = 读(键.配置, {});
    const c = Object.assign({}, 默认配置, 已存);
    if (!Array.isArray(c.时段) || !c.时段.length) c.时段 = 默认配置.时段.slice();
    c.时段 = c.时段.map((段) => {
      const 单数 = Math.min(600, Math.max(1, Math.floor(Number(段 && 段.单数) || 1)));
      return Object.assign({}, 段, { 单数 });
    });

    // 老版本只有一个最小间隔。保留旧用户已有的节奏（固定值），新用户默认 20–48 秒。
    const 有下限 = Object.prototype.hasOwnProperty.call(已存, '间隔下限秒');
    const 有上限 = Object.prototype.hasOwnProperty.call(已存, '间隔上限秒');
    const 旧最小间隔 = Number(已存.最小间隔秒);
    if (!有下限 && Number.isFinite(旧最小间隔) && 旧最小间隔 >= 1) {
      c.间隔下限秒 = 旧最小间隔;
    }
    if (!有上限 && Number.isFinite(旧最小间隔) && 旧最小间隔 >= 1) {
      c.间隔上限秒 = 旧最小间隔;
    }
    c.间隔下限秒 = Math.min(3600, Math.max(1, Math.floor(Number(c.间隔下限秒) || 10)));
    c.间隔上限秒 = Math.min(3600, Math.max(c.间隔下限秒,
      Math.floor(Number(c.间隔上限秒) || c.间隔下限秒)));
    c.每日完成上限 = Math.min(600, Math.max(1,
      Math.floor(Number(c.每日完成上限) || 400)));
    c.抓取模式 = c.抓取模式 === '新标签页' ? '新标签页' : '单标签';

    // 移除旧的 B/A 开关。即便 localStorage 里还留着，也绝不再允许它们生效。
    delete c.最小间隔秒;
    delete c.允许后台页;
    delete c.允许直连;
    return c;
  ...
  // 落后进度时会自动缩短间隔追赶，但不会低于本单随机抽取的间隔下限。
  function 算间隔(配置 = 取配置()) {
    const 段 = 当前时段(配置);
    ...
    const 剩余分 = 时段剩余(段);
    const 下限 = Math.max(1, Number(配置.间隔下限秒) || 10);
    const 上限 = Math.max(下限, Number(配置.间隔上限秒) || 下限);
    const 随机下限 = Math.floor(下限 + Math.random() * (上限 - 下限 + 1));
    // 时段快结束了还有单没做 → 用随机下限冲一冲，做多少算多少
    const 理论秒 = 剩余分 <= 0 ? 随机下限 : (剩余分 * 60) / 剩余单;
    const 抖 = (Number(配置.抖动百分比) || 0) / 100;
    const 抖后 = 理论秒 * (1 + (Math.random() * 2 - 1) * 抖);
    const 秒 = Math.max(随机下限, Math.round(抖后));
    return { 秒, 段, 已完成, 剩余单, 剩余分, 理论秒: Math.round(理论秒),
      随机下限, 间隔下限秒: 下限, 间隔上限秒: 上限 };
  }
  ...
  async function 上报失败(traceId, sku, 原因) {
    try {
      ...
      if (r.code !== 0) {
        ...
        记(`放回任务失败 ${失败原因}`, '警告');
        // 本地记录可能比服务端状态晚一步：任务已完成、已过期作废，
        // 或已被服务端回收后都会不再是 status=2。此时无需继续拿旧记录
        // 阻塞整个任务循环。
        const 已结清 = /任务不存在|已不在领取中|不在领取中|任务可能已被重置|被其他账号领取/i
          .test(失败原因);
        return { 成功: false, 已结清, 原因: 失败原因 };
      }
      记(`已放回任务 ${traceId}：${原因}`);
      return { 成功: true };
    } catch (e) {
  ...
  // 默认仍要求服务端确认 failTask 成功，避免正常情况下遗失仍在领取中的任务。
  // 但服务端已明确说任务不在领取中时，本地记录已经过期，应直接清理；
  // 需要熔断或普通 payload 未命中时允许尽力清理，绝不让旧记录永久锁住启动。
  async function 确认放回当前任务(任务, 原因, 选项 = {}) {
    const 放回 = await 上报失败(任务.traceId, 任务.item || 任务.sku, 原因);
    if (放回.成功 || 放回.已结清) {
      删(键.在跑任务);
      if (放回.已结清) {
        记(`任务 ${任务.traceId} 已不在领取中，已清除本地记录并继续：${放回.原因}`, '警告');
      }
      return true;
    }
    if (选项.允许未确认继续) {
      // 抓取/导航故障时，failTask 的网络错误不应把整个实例锁死。
      // 本地记录必须清掉，否则下次启动会无限重试这条旧任务。
      删(键.在跑任务);
      记(`任务 ${任务.traceId} 放回未确认，已清除本地记录并交给后续节流：${放回.原因 || '未知原因'}`,
        '警告');
      return true;
    }
    const 明细 = 放回.原因 || '未知原因';
  ...
  async function 查当日完成(接单账号) {
    try {
      ...
      // 生产接口通常在 accounts 数组查询时回行数组；有些服务端版本会回
      // PageResult 风格的 data.total。两种都是成功响应，不能把后者误判为故障。
      const 行 = Array.isArray(r.data)
        ? r.data.find((x) => String(x && x.account || '') === 接单账号)
        : null;
      const 候选总数 = 行 ? 行.total
        : (r.data && typeof r.data === 'object' ? r.data.total : undefined);
      const 总数 = 候选总数 == null && Array.isArray(r.data) ? 0 : Number(候选总数);
  ...
  const 服务端完成状态 = { 日期: '', 数: null, 加载中: false, 错误: '', 上次告警: 0 };
  async function 刷新服务端完成() {
    ...
    if (总数 === null) {
      服务端完成状态.数 = null;
      服务端完成状态.错误 = '服务端完成数读取失败，按服务端领取结果继续';
    } else {
      服务端完成状态.数 = 总数;
    }
    刷新界面();
  ...
  // 成功读取时以服务端当天总数为准。本机统计无法覆盖其它设备；但读取接口短暂
  // 波动不能让任务器停摆，拿不到数时记录告警并由服务端 claim 结果继续裁决。
  async function 核验每日完成上限(配置 = 取配置()) {
    const 接单账号 = String(配置.接单账号 || 配置.设备名称 || '').trim();
    const 上限 = Math.min(600, Math.max(1, Number(配置.每日完成上限) || 400));
    if (!接单账号) return { 可领取: false, 原因: '未设置接单账号' };
    ...
    if (总数 === null) {
      服务端完成状态.数 = null;
      服务端完成状态.错误 = '服务端完成数读取失败，按服务端领取结果继续';
      if (现在() - Number(服务端完成状态.上次告警 || 0) >= 5 * 60 * 1000) {
        服务端完成状态.上次告警 = 现在();
        记('服务端今日完成数读取失败，按服务端领取结果继续', '警告');
      }
      刷新界面();
      return { 可领取: true, 统计不可用: true,
        原因: '无法读取服务端今日完成数，按服务端领取结果继续', 上限 };
    }
    服务端完成状态.数 = 总数;
    服务端完成状态.错误 = '';
    刷新界面();
    if (总数 >= 上限) {
      return { 可领取: false, 原因: `已达到每日完成上限（${总数}/${上限}）`, 总数, 上限 };
    }
    return { 可领取: true, 总数, 上限 };
  }
  ...
  // ══════════════════════════════════════ 软导航（同标签，不冷加载）
  // 为什么必须这么做：从首页直连 get_rw 容易被风控拦（error=90309999 +
  ...
  function 找路由() {
    ...
    for (const [名, 取] of 候选) {
      ...
      if (r && (typeof r.push === 'function' || typeof r.back === 'function')) {
        return { 名, 路由: r };
      }
    }
    return null;
  ...
  async function 用路由跳(网址) {
    const 找到 = 找路由();
    if (!找到 || typeof 找到.路由.push !== 'function') {
      return { 成功: false, 机制: '页面router', 原因: '页面未暴露 router.push' };
    }
    try {
  ...
  // 单标签从商品页离开时优先回退浏览器/框架历史，避免把首页当成一次新的正向访问。
  // 只有历史链确实失效时，才用既有软导航作为受控兜底。
  async function 返回来源页(来源地址) {
    if (!来源地址) return { 成功: false, 方式: '无来源页', 原因: '缺少来源页地址' };
    if (路径等价(location.href, 来源地址)) return { 成功: true, 方式: '已在来源页' };

    const 找到 = 找路由();
    if (找到 && typeof 找到.路由.back === 'function') {
      try {
        const r = 找到.路由.back();
        if (r && typeof r.then === 'function') await r.catch(() => {});
        if (await 等路径变成(来源地址)) return { 成功: true, 方式: `router.back(${找到.名})` };
      } catch (_) {}
    }

    if (window.history && typeof history.back === 'function') {
      try {
        history.back();
        if (await 等路径变成(来源地址)) return { 成功: true, 方式: 'history.back' };
      } catch (_) {}
    }

    const 回 = await 软跳到(来源地址);
    return 回.成功
      ? { 成功: true, 方式: `软导航(${回.机制})` }
      : { 成功: false, 方式: '软导航兜底', 原因: 回.原因, 明细: 回.明细 };
  }
  ...
  // 这里是 get_rw 响应的唯一校验口：只有请求、业务码和响应商品三者都
  // 与当前任务一致时才可作为完整详情提交。266900002 + data:null 例外：
  // 先保留原始文本并等待 10 秒，仍没有完整详情时把原始响应上传给后台确认。
  function 校验GetRw响应(文本, 地址, 目标号) {
    ...
    if (错误码 !== null && 错误码 !== 0) {
      const 待确认不存在 = Number(错误码) === 商品不存在错误码 &&
        (对象.data === null || typeof 对象.data === 'undefined');
      return {
        成功: false, 错误码, 待确认不存在,
        原因: 待确认不存在
          ? `get_rw error=${商品不存在错误码}（等待完整详情确认商品状态）`
  ...
  // 抓完优先沿历史返回来源页；历史失效时才软跳兜底，全程不冷加载。
  async function 软跳抓(任务) {
    ...
    try {
      ...
      const 等你做 = 配置.等你做验证码 !== false;
      const 验证码等待毫秒 = Math.max(30, Number(配置.验证码等待秒) || 240) * 1000;
      const 任务截止 = 解析北京时刻(任务.expireDate);
      // 人工等待绝不跨过上游任务的有效期；没给 expireDate 才只用本地等待上限。
      const 验证码截止 = Math.min(
        现在() + 验证码等待毫秒,
        任务截止 && 任务截止 > 现在() ? 任务截止 : Infinity);
      结果 = await 收集.等结果(秒 * 1000, () => 识别验证页面(), {
        等待验证码: 等你做,
        验证码截止,
        发现验证码() {
          已通知验证码 = true;
          记('检测到验证码，保留 get_rw 监听并等待手动完成');
          // 不阻塞监听循环；通知失败由通知函数自己记录，不影响手动验证。
          Promise.resolve(通知验证码()).catch(() => {});
        },
      });
    } finally {
      收集.停();
    ...
    // 先回到本次进入商品页前的来源地址。返回失败不是普通数据超时，
    // 控制器仍滞留在商品页，若继续 claim 会形成导航故障循环，必须熔断。
    if (!路径等价(location.href, 来源页)) {
      const 回 = await 返回来源页(来源页);
      记(回.成功 ? `已返回来源页（${回.方式}）` : `返回来源页失败：${回.原因}`,
        回.成功 ? '常规' : '警告');
      if (!回.成功) {
        return {
          成功: false, 导航故障: true,
          原因: '商品页返回来源页失败：' + (回.原因 || 回.方式),
        };
      }
    }
    ...
    if (结果 && 结果.验证码) {
      return {
        成功: false, 验证码: true, 验证码超时: !!结果.验证码超时,
        已通知: 已通知验证码,
        原因: 结果.验证码超时 ? '等待手动验证码超时' : '商品页跳到了验证码页',
      };
    }
    if (结果 && 结果.待确认不存在) {
      return {
        成功: true, 数据: 结果.数据, 地址: 结果.地址, 错误码: 结果.错误码,
        商品不存在: true,
      };
    }
    if (!结果 || !结果.成功) {
  ...
  function 清理新标签临时记录(单号) {
    for (const 键名 of [键.待抓, 键.回传, 键.状态]) {
      const 值 = 读(键名, null);
      // 无单号时只在启动新任务前清理；运行中绝不碰其他单号的记录。
      if (!单号 || (值 && 值.单号 === 单号)) 删(键名);
    }
  }

  async function 确认关闭子页(子页) {
    if (!子页) return { 成功: false, 原因: '没有可确认的子页句柄' };
    try {
      if (子页.closed === true) return { 成功: true, 方式: '子页已自行关闭' };
    } catch (_) {}
    try {
      if (typeof 子页.close !== 'function') return { 成功: false, 原因: '子页不支持 close()' };
      子页.close();
    } catch (e) {
      return { 成功: false, 原因: '调用子页 close() 失败：' + String(e && e.message || e) };
    }
    const 截止 = 现在() + 1500;
    while (现在() < 截止) {
      try { if (子页.closed === true) return { 成功: true, 方式: '控制器已关闭子页' }; } catch (_) {}
      await 等一下(50);
    }
    return { 成功: false, 原因: '子页 close() 后仍未关闭' };
  }

  // 新标签页模式：主控页始终留在来源页。子页 document-start 监听自然 get_rw，
  // 同源 localStorage 只传带本单单号的结果；控制器确认并关闭子页后才能结束本单。
  async function 新标签抓(任务) {
    const 商品页 = String(任务.taskUrl || '').trim();
    const 目标 = 拼接口地址(任务);
    const 目标号 = 取任务商品号(任务);
    if (!商品页 || !目标 || !目标号) {
      return { 成功: false, 导航故障: true, 原因: '新标签抓取缺少商品页或 get_rw 目标' };
    }
    if (!同源(商品页)) {
      return { 成功: false, 导航故障: true, 原因: '任务商品页跨域，无法建立同源子页' };
    }

    const 配置 = 取配置();
    const 来源页 = location.href;
    const 普通等待毫秒 = Math.max(5, Number(配置.抓取超时秒) || 60) * 1000;
    const 验证码等待毫秒 = Math.max(30, Number(配置.验证码等待秒) || 240) * 1000;
    const 上游截止 = 解析北京时刻(任务.expireDate);
    // 没有 expireDate 时仍给待抓记录有限生命周期，避免旧子页永久劫持角色分流。
    const 任务截止 = 上游截止 && 上游截止 > 现在()
      ? 上游截止 : 现在() + Math.max(普通等待毫秒, 验证码等待毫秒);
    const 单号 = `${任务.traceId || 'task'}-${现在()}-${Math.random().toString(36).slice(2, 10)}`;
    let 截止 = Math.min(现在() + 普通等待毫秒, 任务截止);
    const 待抓 = {
      单号, 模式: '新标签页', traceId: 任务.traceId,
      商品页, 目标, 来源页, 截止, 任务截止,
    };

    // 先清旧值再写本单，回传和状态均必须严格按单号匹配。
    清理新标签临时记录();
    写(键.待抓, 待抓);

    let 子页 = null;
    try { 子页 = window.open(商品页, '_blank'); }
    catch (_) { 子页 = null; }   // iOS/WKWebView 可能抛异常，但标签其实开成了
    const 有句柄 = !!子页;
    // 关子页只在拿到句柄时才有意义；子页抓到数据后本来就会自己 window.close()。
    const 关子页 = () => (有句柄
      ? 确认关闭子页(子页)
      : Promise.resolve({ 成功: true, 方式: '无句柄，靠子页自关' }));

    // Via 把 _blank 降级成「导航当前标签」时，首页控制器就丢了，必须放回并熔断。
    // 这个判断看本页 location，不依赖句柄。
    if (!路径等价(location.href, 来源页)) {
      await 关子页();
      清理新标签临时记录(单号);
      return { 成功: false, 导航故障: true, 原因: '新标签创建导致首页控制器被跳转' };
    }
    // ★ iOS/WKWebView：window.open 常常真开出了标签却返回 null（不把句柄交给 opener）。
    // 绝不能当「被拦」直接失败：那会删掉待抓，子页 document-start 就判不出子页角色、抓不了。
    // 保留待抓，改用同源 localStorage 回传协调；靠回传/状态判断标签是否真的开成，而非句柄。
    // 真被拦（标签没开）时不会有任何本单信号，等待到超时后照旧记为导航故障。
    if (!有句柄) 记('window.open 未给句柄，转无句柄模式（保留待抓，靠回传 + 子页自关协调）', '警告');

    let 已进入验证码 = false;
    let 已通知验证码 = false;
    while (现在() < 截止) {
      const 回传 = 读(键.回传, null);
      if (回传 && 回传.单号 === 单号) {
        const 关闭 = await 关子页();
        清理新标签临时记录(单号);
        if (!关闭.成功) {
          return { 成功: false, 导航故障: true, 原因: '已收到子页回传但无法关闭：' + 关闭.原因 };
        }
        if (回传.成功 && typeof 回传.数据 === 'string' && 回传.数据) {
          // 子页已校验过一次；控制器再按当前单目标复核，防止同源旧记录/异常数据被上传。
          const 校验 = 校验GetRw响应(回传.数据, 回传.地址, 目标号);
          if (校验.成功) return Object.assign({}, 回传, 校验, { 成功: true });
          return { 成功: false, 导航故障: true, 原因: '子页回传数据校验失败：' + 校验.原因 };
        }
        if (回传.待确认不存在 && 回传.数据) {
          return Object.assign({}, 回传, { 成功: true, 商品不存在: true });
        }
        if (已进入验证码 || /验证码/.test(String(回传.原因 || ''))) {
          return {
            成功: false, 验证码: true, 验证码超时: true,
            已通知: 已通知验证码,
            原因: String(回传.原因 || '等待手动验证码超时'),
          };
        }
        return { 成功: false, 导航故障: true, 原因: '子页未取得有效数据：' + String(回传.原因 || '未知原因') };
      }

      const 状态 = 读(键.状态, null);
      if (状态 && 状态.单号 === 单号) {
        if (状态.状态 === '风控') {
          const 关闭 = await 关子页();
          清理新标签临时记录(单号);
        ...
        if (状态.状态 === '验证码') {
          if (配置.等你做验证码 === false) {
            ...
            清理新标签临时记录(单号);
            return {
              成功: false, 验证码: true, 已通知: false,
              导航故障: !关闭.成功,
              原因: '检测到验证码，当前设置为不等待手动验证',
            };
          }
          if (!已进入验证码) {
            已进入验证码 = true;
            已通知验证码 = true;
            截止 = Math.min(现在() + 验证码等待毫秒, 任务截止);
            待抓.截止 = 截止;
            写(键.待抓, 待抓);  // 子页超时轮询会随人工等待窗口延长。
            记('新标签页检测到验证码，保持子页和 get_rw 监听等待手动完成');
            Promise.resolve(通知验证码()).catch(() => {});
          }
        }
      }
    ...
    清理新标签临时记录(单号);
    if (!关闭.成功) {
      return { 成功: false, 导航故障: true, 原因: '新标签页等待超时且无法关闭：' + 关闭.原因 };
    }
    if (已进入验证码) {
      return {
        成功: false, 验证码: true, 验证码超时: true, 已通知: 已通知验证码,
        原因: '等待手动验证码超时',
      };
    }
    return { 成功: false, 导航故障: true, 原因: '新标签页等待回传超时' };
  }
  ...
  function 装本页监听(目标号) {
    ...
    const 收 = (文本, 地址) => {
      ...
      if (校验.成功) {
        命中 = 校验;
      } else if (校验.待确认不存在 && !待确认不存在) {
        // 原始 JSON 是后台确认商品不存在的依据，不能只记错误码后把数据丢掉。
        待确认不存在 = { 数据: 文本, 地址, 错误码: 校验.错误码, 时刻: 现在() };
      }
    };
    ...
    return {
      // 轮询等命中。验证码可以选择保留监听，等用户完成后继续接收同一页面的 get_rw。
      async 等结果(超时毫秒, 取验证类型, 选项 = {}) {
        const 常规截止 = 现在() + 超时毫秒;
        const 验证码截止 = Number(选项.验证码截止);
        let 已进入验证码 = false;
        while (true) {
          if (命中) {
            return 已进入验证码 ? Object.assign({}, 命中, { 经过验证码: true }) : 命中;
          }
          if (待确认不存在 && 现在() >= 待确认不存在.时刻 + 商品不存在确认等待毫秒) {
            return {
              成功: false, 待确认不存在: true,
              数据: 待确认不存在.数据, 地址: 待确认不存在.地址,
              错误码: 待确认不存在.错误码,
              原因: `get_rw error=${商品不存在错误码} 等待 10 秒后仍无完整详情`,
            };
          }
          const 类型 = 取验证类型 && 取验证类型();
          if (类型 === '风控') return { 成功: false, 验证码: false, 风控: true };
          if (类型 === '验证码') {
            if (!选项.等待验证码) return { 成功: false, 验证码: true, 风控: false };
            if (!已进入验证码) {
              已进入验证码 = true;
              try { if (typeof 选项.发现验证码 === 'function') 选项.发现验证码(); } catch (_) {}
            }
          }
          const 截止 = 已进入验证码 && Number.isFinite(验证码截止)
            ? 验证码截止 : 常规截止;
          if (现在() >= 截止) {
            return 已进入验证码
              ? { 成功: false, 验证码: true, 风控: false, 验证码超时: true }
              : { 成功: false, 原因: '等 get_rw 超时' };
          }
          await 等一下(150);
        }
      },
  ...
  // 抓取总入口：默认 C 单标签软导航，可选新标签页。B 旧后台页和 A 直连均已移除，
  // 避免冷加载或伪造 Referer 导致风控。任何导航设施故障都交给主循环熔断。
  async function 抓数据(任务) {
    const 配置 = 取配置();
    if (配置.抓取模式 === '新标签页') {
      const 新标签结果 = await 新标签抓(任务);
      if (新标签结果.成功) {
        清导航熔断();
        记(`新标签页抓到数据（${新标签结果.数据.length} 字符）`);
        return 新标签结果;
      }
      if (!新标签结果.验证码 && !新标签结果.风控) {
        记(`新标签页抓取失败：${新标签结果.原因}`, '警告');
      }
      return 新标签结果;
    }
    if (配置.软导航优先 === false) {
      return {
        成功: false, 导航故障: true,
        原因: 'C 软导航已关闭，当前版本没有其它抓取路线',
      };
    }
    const c = await 软跳抓(任务);
    if (c.成功) {
      清导航熔断();
      记(`软导航抓到数据（${c.数据.length} 字符）`);
      return c;
    }
    if (!c.验证码 && !c.风控 && !c.待确认不存在) {
      记(`软导航路线失败：${c.原因}`, '警告');
    }
    return c;
  }
  ...
  async function 跑一单() {
    ...
    if (!抓.成功) {
      ...
      if (抓.验证码) {
        // 开启人工验证时，监听会一直保留到验证码窗口结束；到期后放回/清理这一单，
        // 休眠两小时再继续，避免在同一会话里连续撞验证码。
        if (!抓.已通知) {
          计一次('验证码');
          await 通知验证码();
        }
        const 人工等待超时 = 抓.验证码超时 && 配置.等你做验证码 !== false;
        记(`${人工等待超时 ? '等待手动验证码超时' : '检测到验证码'}，放回任务：${抓.原因}`, '警告');
        await 确认放回当前任务(
          任务, 'shopee_captcha_pending', { 允许未确认继续: true });
        计一次('失败');
        const 熔断 = 抓.导航故障 ? 触发导航熔断('验证码子页关闭/导航异常：' + 抓.原因) : null;
        const 验证码冷却 = 人工等待超时 ? 触发验证码冷却(抓.原因) : null;
        return 人工等待超时
          ? {
            结果: '验证码超时',
            等待毫秒: Math.max(验证码冷却.截止 - 现在(), 熔断 ? 熔断.截止 - 现在() : 0),
          }
          : { 结果: '验证码跳过', 立即下一单: true };
      }
      if (抓.导航故障) {
        // 不同于「本次 get_rw 没数据」：导航本身已不可用，若立即 claim 必然高速循环。
        await 确认放回当前任务(
          任务, 'navigation_failure: ' + 抓.原因, { 允许未确认继续: true });
        const 熔断 = 触发导航熔断(抓.原因);
        计一次('失败');
        return { 结果: '导航故障', 导航熔断: true, 熔断截止: 熔断.截止 };
      }
      记(`抓取失败：${抓.原因}`, '警告');
      await 确认放回当前任务(
        任务, 'payload_timeout: ' + 抓.原因, { 允许未确认继续: true });
      计一次('失败');
      return { 结果: '抓取失败', 立即下一单: true };
    }

    // 业务错误码检查：风控页/异常响应不该当成有效数据提交。
    // 已经等待 10 秒确认的「不存在商品」例外：需上传原始 JSON 供后台落库判断。
    if (!抓.商品不存在 && !可忽略错误码(抓.错误码)) {
    ...
    if (交.成功) {
      ...
      const 段 = 当前时段(配置);
      计一次('完成', 段 ? 时段键(段) : null);
      记(`✅ 上传成功 ${任务.traceId}` + (抓.商品不存在 ? '（商品不存在响应）'
        : (抓.经过验证码 ? '（做过验证码）' : '')));
      if (抓.经过验证码) {
  ...
  async function 清理遗留() {
    ...
    // 若上次是导航失败触发的页面重建，主循环随后会进入熔断冷却；
    // 此时 failTask 的瞬时网络故障不能重新把脚本锁死。
  ...
  async function 主循环() {
    ...
    try {
      ...
      while (!停止请求 && 是运行中()) {
        ...
        if (验证码冷却) {
          下次时刻 = 验证码冷却.截止;
          const 剩分钟 = Math.max(1, Math.ceil((验证码冷却.截止 - 现在()) / 60000));
          记(`验证码人工等待冷却中（约 ${剩分钟} 分钟）：${验证码冷却.原因}`, '警告');
          刷新界面();
          await 睡到(下次时刻, 本轮配置版本);
          continue;
        }
        const 间隔 = 算间隔(配置);
        ...
        // 熔断必须位于每日统计与 claim 之前：冷却期不能产生任何领任务请求。
        const 熔断 = 取导航熔断();
        if (熔断 && 熔断.截止 > 现在()) {
          下次时刻 = 熔断.截止;
          const 剩分钟 = Math.max(1, Math.ceil((熔断.截止 - 现在()) / 60000));
          记(`导航熔断冷却中（L${熔断.级别}，约 ${剩分钟} 分钟）：${熔断.原因}`, '警告');
          刷新界面();
          await 睡到(下次时刻, 本轮配置版本);
          continue;
        }

        // 只在即将 claim 时核验，时段外和本时段已满都不会额外请求接口。
        const 上限核验 = await 核验每日完成上限(配置);
        if (!上限核验.可领取) {
          下次时刻 = 0;
          记(上限核验.原因, '警告');
          设运行(false);
        ...
        if (Number.isFinite(Number(r.等待毫秒)) && Number(r.等待毫秒) > 0) {
          const 等待毫秒 = Number(r.等待毫秒);
          下次时刻 = 现在() + 等待毫秒;
          记(`验证码等待超时，${等待毫秒 / 60 / 60 / 1000} 小时后继续领取任务`, '警告');
          刷新界面();
          await 睡到(下次时刻, 本轮配置版本);
          continue;
        }
        ...
        if (r.导航熔断) {
          // 下一轮开头会读到熔断并睡到截止；这里不经过正常间隔，更不能 claim。
          下次时刻 = 0;
          刷新界面();
          continue;
        }

        // 当前任务未取得有效数据时，不占完成配额；已尽力放回或清理本地记录后，
        // 不等待本时段节奏，直接领取下一条。
        if (r.立即下一单) {
          下次时刻 = 0;
          记('本单未获取有效数据，直接领取下一单');
  ...
  async function 睡到(时刻, 开始配置版本 = 配置版本) {
    while (现在() < 时刻) {
      if (停止请求 || !是运行中()) return;
      if (配置版本 !== 开始配置版本) return;
  ...
  function 子页面模式(待抓) {
    ...
    const 查超时 = () => {
      ...
      if (待确认不存在 && 现在() >= Math.min(
        截止, 待确认不存在.时刻 + 商品不存在确认等待毫秒)) {
        交差({
          成功: false, 待确认不存在: true,
          数据: 待确认不存在.数据, 地址: 待确认不存在.地址,
          错误码: 待确认不存在.错误码,
  ...
  async function 切换任务() {
    ...
    刷新服务端完成();
    // 手动点「启动任务」是明确的重试意图（尤其 iOS 手机无控制台，只能靠这个解冻）。
    // 清掉上一轮留下的导航熔断冷却 + 验证码等待冷却；两者都可能在停机后残留，
    // 不清就会「一启动又立刻睡到冷却截止」，表现为怎么点都不跑。
    // 若这次仍失败会重新触发对应冷却，不会造成高速重领。
    解冻冷却('手动启动');
    设运行(true);
    记('开始跑任务');
    主循环();
  ...
  function 建配置卡(体) {
    ...
    造('label', { 文: '抓取模式' }, 卡);
    元素.抓取模式 = 造('select', {}, 卡);
    造('option', { value: '单标签', 文: '单标签（默认，软导航后历史返回）' }, 元素.抓取模式);
    造('option', { value: '新标签页', 文: '新标签页（首页不跳转，抓完关闭子页）' }, 元素.抓取模式);
    元素.抓取模式.value = c.抓取模式;
    造('div', {
      类: '提',
      文: '新标签页模式每次只开一个商品页；Via 可能前台显示或拦截新页。创建、回传、关闭失败会自动放回并进入冷却，不会高速领单。',
    }, 卡);

    const 行 = 造('div', { 类: '行' }, 卡);
    const 左 = 造('div', {}, 行), 右 = 造('div', {}, 行);
    造('label', { 文: '随机间隔下限(秒)' }, 左);
    元素.间隔下限秒 = 造('input', {
      type: 'number', min: '1', max: '3600', value: String(c.间隔下限秒),
    }, 左);
    造('label', { 文: '随机间隔上限(秒)' }, 右);
    元素.间隔上限秒 = 造('input', {
      type: 'number', min: '1', max: '3600', value: String(c.间隔上限秒),
    }, 右);

    const 配额行 = 造('div', { 类: '行' }, 卡);
    const 配额左 = 造('div', {}, 配额行), 配额右 = 造('div', {}, 配额行);
    造('label', { 文: '账号每日完成上限' }, 配额左);
    元素.每日完成上限 = 造('input', {
      type: 'number', min: '1', max: '600', value: String(c.每日完成上限),
    }, 配额左);
    造('label', { 文: '时段节奏抖动(%)' }, 配额右);
    元素.抖动百分比 = 造('input', { type: 'number', min: '0', max: '80', value: String(c.抖动百分比) }, 配额右);
    造('div', {
      类: '提',
      文: '每单在下限～上限内随机取最短等待；时段均摊需要更慢时会自动延长。' +
        '每日上限按服务端账号当天总完成数计算，范围 1～600。保存后会立刻按新设置重算。',
    }, 卡);
    ...
    // ── 单标签模式只保留已验证的 C 软导航，避免任何冷加载或直连路线触发风控。
    造('label', { 文: '单标签抓取路线' }, 卡);
    const 勾 = (父, 文字, 选中) => {
    ...
    元素.软导航优先 = 勾(卡, 'C 软导航到商品页（默认，Referer 真实、不冷加载）',
      c.软导航优先 !== false);
    元素.仅软导航 = 勾(卡, '　└ 严格模式：软导航失败就放回任务',
      c.仅软导航 === true);
    造('div', {
      类: '提',
      文: '已移除旧 B 后台页和 A 直连 get_rw。单标签 C 让页面自己发请求，' +
        'Referer/签名都由页面真实生成；新标签模式也只监听页面自然请求。',
    }, 卡);
    ...
    造('div', {
      类: '提',
      文: '勾上：遇验证码会推送通知，并把当前页留着等你手动做，' +
        '做完自动抓数据并上传，这一单不算失败。手动等待超时后会放回/清理当前任务，' +
        '固定休眠 2 小时再继续领取。' +
        '不勾：直接放回/清理并领取下一单。等待上限受任务过期时间收敛（shopee 只留 5 分钟）。',
    }, 卡);
    ...
    保存.addEventListener('click', () => {
      ...
      c2.抓取模式 = 元素.抓取模式.value === '新标签页' ? '新标签页' : '单标签';
      c2.间隔下限秒 = Math.min(3600, Math.max(1,
        Math.floor(Number(元素.间隔下限秒.value) || 10)));
      c2.间隔上限秒 = Math.min(3600, Math.max(c2.间隔下限秒,
        Math.floor(Number(元素.间隔上限秒.value) || c2.间隔下限秒)));
      c2.每日完成上限 = Math.min(600, Math.max(1,
        Math.floor(Number(元素.每日完成上限.value) || 400)));
      ...
      记('配置已保存');
      // 配置版本已递增，主循环若正在等待会立刻退出本轮睡眠并按新设置重算。
      刷新服务端完成();
  ...
  function 建时段卡(体) {
    ...
    造('div', {
      ...
      文: '间隔不用填：脚本按「时段剩余时间 ÷ 剩余单数」自动算，' +
        '把单量均匀铺满整个时段。落后了会自动加快，但不低于随机间隔下限。' +
        '每段单数可设 1～600，并会受到每日完成上限兜底。' +
    ...
    存.addEventListener('click', () => {
      ...
      for (const 行元素 of 元素.时段容器.children) {
        ...
        if (!Number.isFinite(n) || n < 1 || n > 600) {
          元素.时段态.textContent = '单数要是 1～600 的整数';
          return;
        }
  ...
  function 画时段行() {
    ...
    c.时段.forEach((段, i) => {
      ...
      const 数 = 造('input', { type: 'number', min: '1', max: '600', value: String(段.单数) }, 行);
  ...
  function 画界面() {
    ...
    const 服务端完成 = 服务端完成状态.日期 === 北京日期()
      ? 服务端完成状态.数 : null;
    const 每日上限 = 取配置().每日完成上限;

    if (外部开关) {
      外部开关.classList.toggle('跑', 跑着);
      外部开关.textContent = 跑着
        ? `停止任务\n今日完成 ${服务端完成 === null ? '—' : 服务端完成}/${每日上限}`
        : '启动任务';
    }
  ...
  function 刷新倒计时() {
    ...
    const 验证码冷却 = 取验证码冷却();
    if (验证码冷却) {
      const 剩 = Math.max(1, Math.ceil((验证码冷却.截止 - 现在()) / 1000));
      元素.状态.textContent = `验证码等待冷却：${剩} 秒后继续（${验证码冷却.原因}）`;
      return;
    }
    const 熔断 = 取导航熔断();
    if (熔断 && 熔断.截止 > 现在()) {
      const 剩 = Math.max(1, Math.ceil((熔断.截止 - 现在()) / 1000));
      元素.状态.textContent = `导航熔断 L${熔断.级别}：${剩} 秒后恢复探测（${熔断.原因}）`;
      return;
    }
    const 间隔 = 算间隔(配置);
  ...
  if (角色.角色 === '中继') {
    ...
    const 启动 = () => {
      ...
      if (退化) {
        ...
        // 冷加载是最严重的一类软导航故障。旧版本直接停机，恢复后又可能被用户
        // 连续点启动；现在放回遗留任务并按 10m→30m→2h 受控恢复探测。
        const 熔断 = 触发导航熔断(剩 === 0
          ? '四种软导航机制均已退化成冷加载'
          : `${退化.机制} 退化成冷加载，还剩 ${剩} 种机制`);
        记(`已进入导航熔断 L${熔断.级别}，冷却后仅恢复探测一次`, '错误');
    ...
    window.__跑任务__ = {
      ...
      领任务, 提交任务, 上报失败, 查当日完成,
      抓数据, 软跳抓, 新标签抓,
      软跳到, 返回来源页, 装本页监听, 识别验证页面,
      取导航熔断, 触发导航熔断, 清导航熔断,
      取验证码冷却, 触发验证码冷却, 清验证码冷却, 解冻冷却,
      // 无控制台时可忽略；有控制台时一键清掉所有会拦住领单的冷却/熔断。
      解冻() { return 解冻冷却('手动解冻'); },
      软跳诊断: () => ({
        优选机制: 读软跳优选(),
        黑名单: 读软跳黑名单(),
      ...
      切换任务,
      // ⚠ 开始() 等同「自动恢复」路径：绝不清冷却/熔断（清了就等于熔断失效）。
      // 要手动解冻请点「启动任务」(切换任务) 或调用 解冻()。
