...
//
// 抓数据只保留 C 软导航（默认）—— 在当前标签页软跳到商品页，让【页面自己】发 get_rw，
...
(function () {
  ...
  function 取配置() {
    ...
    // 老版本只有一个最小间隔。保留旧用户已有的节奏（固定值），新用户默认 10–30 秒。
  ...
  // 抓取不到有效数据时也允许尽力放回后继续下一单，避免短暂服务端波动停机。
  async function 确认放回当前任务(任务, 原因, 选项 = {}) {
    ...
    if (选项.允许未确认继续) {
      // 抓取本身失败时，failTask 的网络错误不应把整个实例锁死。
      // 本地记录必须清掉，否则下次启动会无限重试这条旧任务。
      删(键.在跑任务);
      记(`任务 ${任务.traceId} 放回未确认，按“抓取失败继续”策略直接下一单：${放回.原因 || '未知原因'}`,
        '警告');
      return true;
    }
    const 明细 = 放回.原因 || '未知原因';
  ...
  // 每次领取前都以服务端当天总数为准：本机统计无法覆盖其它设备，不能拿来做上限。
  // 无法读取时宁可停住，也不能在未知完成数的情况下冒险超额。
  async function 核验每日完成上限(配置 = 取配置()) {
    ...
    if (总数 === null) {
      ...
      服务端完成状态.错误 = '服务端完成数读取失败';
      刷新界面();
      return { 可领取: false, 原因: '无法读取服务端今日完成数，为避免超额已停止' };
    }
    服务端完成状态.数 = 总数;
    服务端完成状态.错误 = '';
    刷新界面();
  ...
  // 抓取总入口：只允许 C 软导航。B 后台标签页和 A 直连均已移除，
  // 避免冷加载或伪造 Referer 导致风控。严格模式保留为显式安全策略；
  // 即使关闭它，本版本也不会重新启用任何旧路线。
  async function 抓数据(任务) {
    const 配置 = 取配置();
    if (配置.软导航优先 === false) {
      return { 成功: false, 原因: 'C 软导航已关闭，当前版本没有其它抓取路线' };
    }
  ...
  function 建配置卡(体) {
    ...
    // ── 只保留已验证的 C 软导航，避免任何冷加载或直连路线触发风控。
    造('label', { 文: '抓取路线' }, 卡);
    const 勾 = (父, 文字, 选中) => {
    ...
    造('div', {
      类: '提',
      文: '已移除 B 后台标签页和 A 直连 get_rw。C 让页面自己发请求，' +
        'Referer/签名都由页面真实生成。',
    }, 卡);
